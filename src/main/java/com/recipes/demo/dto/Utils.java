...
public class Utils {
	 ...
	 * 
	 * boolean hasExpired = true; try { Claims claims =
	 * Jwts.parser().setSigningKey(SecurityConstants.getTokenSecret()).
	 * parseClaimsJws(token).getBody();
	 * 
	 * Date tokenExpiration = claims.getExpiration(); Date today = new Date();
	 * hasExpired = tokenExpiration.before(today); } catch (ExpiredJwtException ex)
	 * { hasExpired = true; }
	 * 
	 * return hasExpired; }
	 * 
	 ...
	 * 
	 * String token = Jwts.builder().setSubject(userId) .setExpiration(new
