
package com.recipes.demo.controller;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.MediaType;
import org.springframework.web.bind.annotation.CrossOrigin;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import com.recipes.demo.request.RecipeRequest;
import com.recipes.demo.response.RecipeRest;
import com.recipes.demo.service.RecipeService;

@CrossOrigin(origins = "http://localhost:4200")
@RestController
@RequestMapping("/recipes") // http://localhost:8080/recipes
public class RecipesController {

	@Autowired
	RecipeService recipeService;
	
	@GetMapping
	public List<RecipeRest> getRecipes() {
		return recipeService.getRecipes();
	}

	@PostMapping(consumes = { MediaType.APPLICATION_XML_VALUE, MediaType.APPLICATION_JSON_VALUE })
	public List<RecipeRest> createRecipe(@RequestBody List<RecipeRequest> recipeDetails) {
		return recipeService.createRecipe(recipeDetails);
	}

	@PutMapping
	public String updateRecipes() {
		return "update user was called";
	}

	@DeleteMapping
	public String deleteRecipes() {
		return recipeService.deleteRecipes();
	}
	
}
